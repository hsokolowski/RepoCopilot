...
def _extract_files_from_patch(patch_markdown: str) -> List[str]:
    ...
    for line in patch_markdown.splitlines():
        ...
        # 4) Diff headers: "--- a/agent/core/prompts.py", "+++ b/scripts/build_index.py"
        diff_match = re.match(r"^(?:---|\+\+\+)\s+[ab][\\/](?P<path>\S+)", line)
        if diff_match:
            path = diff_match.group("path")
            files.add(path)
...
def generate_pr_payload(
    ...
    if patch_markdown:
        body_lines.append("## Patch (preview)")
        body_lines.append("```diff")
        body_lines.append(patch_markdown)
        body_lines.append("```")
        body_lines.append("")
        body_lines.append("")
